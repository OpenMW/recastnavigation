...
namespace
{
	// Stable per-column batches preserve addSpan merge semantics on empty heightfields.
                          ...
                          float axisOffset, float& minX, float& maxX)
{
	...
	for (int inVertA = 0, inVertB = inVertsCount - 1; inVertA < inVertsCount; inVertB = inVertA, ++inVertA)
	{
		...
		if (!sameSide)
		{
			...
			outRow[rowVert * 2 + 0] = inVerts[inVertB * 3 + 0] + (inVerts[inVertA * 3 + 0] - inVerts[inVertB * 3 + 0]) * s;
			outRow[rowVert * 2 + 1] = inVerts[inVertB * 3 + 1] + (inVerts[inVertA * 3 + 1] - inVerts[inVertB * 3 + 1]) * s;
			outRemainder[remainderVert * 3 + 0] = outRow[rowVert * 2 + 0];
			outRemainder[remainderVert * 3 + 1] = outRow[rowVert * 2 + 1];
			outRemainder[remainderVert * 3 + 2] = inVerts[inVertB * 3 + 2] + (inVerts[inVertA * 3 + 2] - inVerts[inVertB * 3 + 2]) * s;
			includeRowX(outRow[rowVert * 2], rowVert, minX, maxX);
			rowVert++;
			remainderVert++;
			...
			if (inVertADelta > 0)
			{
				outRow[rowVert * 2 + 0] = inVerts[inVertA * 3 + 0];
				outRow[rowVert * 2 + 1] = inVerts[inVertA * 3 + 1];
				includeRowX(outRow[rowVert * 2], rowVert, minX, maxX);
				rowVert++;
			}
...
// Preserve clip order while reducing the consumed side directly to its Y bounds.
	...
	float* outRemainder, int* outRemainderCount, const float axisOffset)
{
	...
	int cellVert = 0;
	int remainderVert = 0;
	float inVertBDelta = axisOffset - inVerts[(inVertsCount - 1) * 2];
	for (int inVertA = 0, inVertB = inVertsCount - 1;
		inVertA < inVertsCount; inVertB = inVertA, ++inVertA)
	{
		const float inVertADelta = axisOffset - inVerts[inVertA * 2];
		const bool sameSide = (inVertADelta >= 0) == (inVertBDelta >= 0);
		if (!sameSide)
		{
			const float s = inVertBDelta / (inVertBDelta - inVertADelta);
			outRemainder[remainderVert * 2 + 0] = inVerts[inVertB * 2 + 0]
				+ (inVerts[inVertA * 2 + 0] - inVerts[inVertB * 2 + 0]) * s;
			outRemainder[remainderVert * 2 + 1] = inVerts[inVertB * 2 + 1]
				+ (inVerts[inVertA * 2 + 1] - inVerts[inVertB * 2 + 1]) * s;
			includeSpanHeight(outRemainder[remainderVert * 2 + 1], cellVert, spanMin, spanMax);
			++remainderVert;

			if (inVertADelta > 0)
			{
				includeSpanHeight(inVerts[inVertA * 2 + 1], cellVert, spanMin, spanMax);
			}
			else if (inVertADelta < 0)
			{
				outRemainder[remainderVert * 2 + 0] = inVerts[inVertA * 2 + 0];
				outRemainder[remainderVert * 2 + 1] = inVerts[inVertA * 2 + 1];
				++remainderVert;
			}
		}
		else
	...
	rcTempVector<RasterizedSpan>* rasterizedSpans)
{
	...
	const float spanMinCell = spanMin * inverseCellHeight;
	const float spanMaxCell = spanMax * inverseCellHeight;
	const int spanMinCellFloor = (int)spanMinCell;
	const int spanMaxCellFloor = (int)spanMaxCell;
	const unsigned short spanMinCellIndex
		= (unsigned short)rcClamp(spanMinCellFloor, 0, RC_SPAN_MAX_HEIGHT);
	const unsigned short spanMaxCellIndex = (unsigned short)rcClamp(
		spanMaxCellFloor + ((float)spanMaxCellFloor < spanMaxCell), (int)spanMinCellIndex + 1,
		RC_SPAN_MAX_HEIGHT);
                         ...
                         const int flagMergeThreshold, rcTempVector<RasterizedSpan>* rasterizedSpans)
{
	...
	{
		float* inRow = rowBuf;
		float* cellRemainder = rowBuf + 7 * 2;
		...
		if (z == z1 && triBBMax[2] <= cellZ + cellSize)
		{
			...
			{
				inRow[vert * 2 + 0] = in[vert * 3 + 0];
				inRow[vert * 2 + 1] = in[vert * 3 + 1];
				includeRowX(inRow[vert * 2], vert, minX, maxX);
			}
		}
		else
		{
			dividePolyRow(in, nvIn, inRow, &nvRow, zRemainder, &nvIn, cellZ + cellSize, minX, maxX);
			rcSwap(in, zRemainder);
		}
		...
		for (int x = x0; x <= x1; ++x)
		{
			...
			if (x == x1 && maxX <= cx + cellSize)
			{
				...
				if (nv >= 3)
				{
					...
					{
						spanMin = rcMin(spanMin, inRow[vert * 2 + 1]);
						spanMax = rcMax(spanMax, inRow[vert * 2 + 1]);
					}
				}
			}
			else
			{
				dividePolyCell(inRow, nv2, &nv, spanMin, spanMax, cellRemainder, &nv2, cx + cellSize);
				rcSwap(inRow, cellRemainder);
			}
